...
int main(int argc, char *argv[])
{
    ...
    auto const s = 3;
    auto const kernel = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(2*s + 1, 2*s + 1), cv::Point(s, s));
    cv::dilate(edges, edges, kernel);
    cv::erode(edges, edges, kernel);

    std::vector<std::vector<cv::Point>> vecTmp;
    std::vector<cv::Vec4i> hierarchy;
    cv::findContours(edges, vecTmp, hierarchy, cv::RETR_TREE, cv::CHAIN_APPROX_SIMPLE);

    std::vector<std::vector<DGtal::Z2i::RealPoint> > vecPts;
    for (auto const& v : vecTmp) {
        if (cv::contourArea(v) < 300)
            continue;

        auto & pts = vecPts.emplace_back();
        //
        for (auto const& p : v)
    ...
    for (size_t it_contour = 0;it_contour != vecPts.size();it_contour++)
    {
        ...
        if(vecPts.at(it_contour).size()<5*maxMT) {
            vector<double> oneVecMT;
            for (size_t it = 0;it != vecPts.at(it_contour).size(); it++)
                oneVecMT.push_back(1);
